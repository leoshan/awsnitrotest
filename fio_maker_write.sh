...
for i in `ls fio_res*`
do
	iops=`head -n 29 $i | tail -n 4 | grep iops | awk -F',' '{print $3}' |awk -F'=' '{print $2}'`
	bw=`head -n 29 $i | tail -n 4 |grep iops | awk -F',' '{print $2}' |awk -F'=' '{print $2}'`
	bwdata=`head -n 29 $i | tail -n 4 | grep iops | awk -F',' '{print $2}' |awk -F'=' '{print $2}'|awk -F"[KM]" '{print $1}'`
	bwunit=${bw:0-4:4}

	delaydata=`head -n 29 $i | tail -n 4 | grep -w lat | grep -w avg | awk -F',' '{print $3}'|awk -F'=' '{print $2}'`
	delayunit=`head -n 29 $i | tail -n 4 | grep -w lat | head -n 1 | awk -F'(' '{print $2}' |  awk -F')' '{print $1}'`
